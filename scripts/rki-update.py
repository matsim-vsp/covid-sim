...
print(fname)
munich = csv >> filter_by(X.Landkreis=='SK München') \
...
print(fname)
heinsberg = csv >> filter_by(X.Landkreis=='LK Heinsberg') \
             >> group_by(X.Landkreis, X.year, X.month, X.day) \
             >> summarize(cases=X.AnzahlFall.sum()) \
             >> arrange(X.year, X.month, X.day)
